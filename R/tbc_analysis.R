...
# install.packages(c("future", "future.apply", "dplyr", "data.table", "pheatmap", "RANN", "RColorBrewer", "latex2exp", "showtext"))
...
#' relationship between the transcripts of a single gene and the existing cell types.
#'
#' @param gene The name of the gene (string) to process.
#' @param coords_df A data frame of transcript coordinates with columns 'x', 'y', 'feature_name'.
#' @param cell_metadata_df A data frame of cell metadata with columns 'x', 'y', 'celltype'.
#' @param celltype_coph_df The pre-computed cophenetic distance matrix between cell types.
#' @param coph_method The hierarchical clustering method (e.g., "average").
#' @return A one-row data frame of cophenetic distances for the gene, or NULL on error.
process_gene <- function(gene, coords_df, cell_metadata_df, celltype_coph_df, coph_method) {
  tryCatch({
    ...
    if (nrow(gene_coords) == 0) {
      # If the gene is not in the original celltype matrix (e.g., a gene with no cells),
      # return a row of NAs.
      ...
      # Otherwise, return the pre-computed row from the celltype-by-celltype matrix,
      # removing the column corresponding to the gene itself.
    ...
    # The result we need is the row for our gene from the 'row' (Searcher) matrix
...
#'
#' This function loads spatial data, computes a base celltype-by-celltype spatial
#' distance matrix, and then iterates through all genes in parallel to compute
#' the spatial distance of each gene's transcripts to all cell types.
#'
#' @param folder Path to the Xenium data folder.
...
#' @param df An optional data frame to map cell_id to a new "group" annotation.
#' @return Creates output files in the specified directory.
transcript_by_cell_analysis <- function(folder,
                                        sample_name = NULL,
                                        output_folder = NULL,
                                        coph_method = "average",
                                        n_jobs = 4,
  ...
  # 1. `cell_metadata`: Contains cell info with columns 'x', 'y', 'cell_id', and a cluster/celltype column.
  # 2. `coords`: Contains transcript info with columns 'x', 'y', 'feature_name', 'cell_id'.
  ...
  # # This data might come from a different file or object slot
  ...
  cell_metadata <- data.frame(
      cell_id = 1:100,
      Cluster = sample(paste0("Type", 1:5), 100, replace = TRUE),
      x = runif(100, 0, 500),
      y = runif(100, 0, 500)
  )
  coords <- data.frame(
      x = runif(1000, 0, 500),
      y = runif(1000, 0, 500),
      feature_name = sample(paste0("Gene", 1:20), 1000, replace = TRUE),
      cell_id = sample(1:100, 1000, replace = TRUE)
  )
  # Mimic adata.var.index
  genes <- unique(coords$feature_name)

  # Filter out control probes from transcripts
  coords <- coords[!grepl("NegControl|Unassigned", coords$feature_name, ignore.case = TRUE), ]

  # Prepare the cell metadata data frame (`adata_obs` equivalent)
  # The final data frame needs columns 'x', 'y', and 'celltype'.
  if (!is.null(df) && "cell_id" %in% colnames(df) && "group" %in% colnames(df)) {
    cell_metadata <- merge(cell_metadata, df[, c("cell_id", "group")], by = "cell_id", all.x = TRUE)
    # Use 'group' as the celltype annotation, keep original 'Cluster' for cells not in df
    cell_metadata$celltype <- ifelse(is.na(cell_metadata$group), cell_metadata$Cluster, cell_metadata$group)
  } else {
    cell_metadata$celltype <- cell_metadata$Cluster
  }
  ...
  write.csv(celltype_coph,
            file = file.path(output_folder, paste0("StructureMap_table_", sample_name, ".csv")),
            row.names = TRUE)


  # --- Parallel processing of genes ---
  message(sprintf("--- Initializing %d workers to process %d genes ---", n_jobs, length(genes)))
  # Setup parallel backend
  plan(multisession, workers = n_jobs)

  # Use future_lapply for parallel execution with a progress bar
  # It passes all necessary objects to the workers automatically.
  ...
  # Using data.table::rbindlist is very fast
  final_df <- data.table::rbindlist(results_list, idcol = "gene")
  setDF(final_df) # Convert back to standard data frame
  rownames(final_df) <- final_df$gene
  final_df$gene <- NULL


  # Write the final combined table
  output_csv <- file.path(output_folder, paste0("t_and_c_result_", sample_name, ".csv"))
...
if (!interactive()) {
  ...
  if (length(args) == 0) {
    message("Usage: Rscript tbc_analysis.R <path_to_data_folder>")
    # Use current directory as a default for demonstration
    args[1] <- "."
  }

  folder_path <- args[1]
  message(paste("Running analysis on folder:", folder_path))

  # Call the main function
  transcript_by_cell_analysis(
    folder = folder_path,
    n_jobs = 4 # Adjust number of jobs as needed
  )
}
